"""
agent_routing.py — Multi-agent routing layer + RAG retrieval engine.
...
Kiến trúc async:
  ...
  - Qdrant query dùng AsyncQdrantClient → hoàn toàn async
"""
...
# ---------------------------------------------------------------------------
# Thinking stream helper (used by server.py)
# ---------------------------------------------------------------------------
...
def create_router_agent(
    ...
    global _router_llm_instance, _grader_llm, _rewriter_llm
    _router_llm_instance = router_llm
    _grader_llm          = non_streaming_llm
    _rewriter_llm        = non_streaming_llm
